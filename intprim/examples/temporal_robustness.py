...
def temporal_robustness():
    ...
    train_basis_model = basis_model.GaussianBasisModel(8)
    phase_velocities = []
    translation_variance = 4.0
    train_noise_variance = 0.001
    temporal_variance = 30
    ...
    for demo in range(30):
        ...
        demonstration_length = int(np.round(np.random.normal(xdata.shape[0], temporal_variance)))
        domain = np.linspace(0, 1, xdata.shape[0], dtype = np.float64)
        weights = train_basis_model.fit_basis_functions_linear_closed_form(domain, np.array([xdata, ydata]).T).T
        domain = np.linspace(0, 1, demonstration_length, dtype = np.float64)
        train_trajectory = np.zeros((2, demonstration_length))
        for idx in range(demonstration_length):
            basis_matrix = train_basis_model.get_block_diagonal_basis_matrix(domain[idx : idx + 1], 2)

            dist_mean = np.dot(basis_matrix.T, weights.flatten()).flatten()

            train_trajectory[:, idx] = dist_mean

        # Apply a random translation
        translation_noise = np.random.normal(0.0, translation_variance)
        train_trajectory = train_trajectory + translation_noise
        ...
        phase_velocities.append(1.0 / demonstration_length)

    demonstration_length = 80
    domain = np.linspace(0, 1, xdata.shape[0], dtype = np.float64)
    weights = train_basis_model.fit_basis_functions_linear_closed_form(domain, np.array([xdata, ydata]).T).T
    domain = np.linspace(0, 1, demonstration_length, dtype = np.float64)
    test_trajectory = np.zeros((2, demonstration_length))
    for idx in range(demonstration_length):
        basis_matrix = train_basis_model.get_block_diagonal_basis_matrix(domain[idx : idx + 1], 2)

        dist_mean = np.dot(basis_matrix.T, weights.flatten()).flatten()

        test_trajectory[:, idx] = dist_mean

    # Note that we're translating by the variance to simulate an extreme case, rather than sampling from a normal distribution.
    test_trajectory = test_trajectory - translation_variance

    test_noise_variance = 0.0029
    ...
    observation_noise = np.array([[100000.0, 0.0], [0.0, test_noise_variance]])
    ...
    for observed_index in range(1, 80, 8):
        gen_trajectory, phase = primitive.generate_probable_trajectory_recursive(test_trajectory_partial[:, prev_observed_index:observed_index], observation_noise, num_samples = 100 - observed_index)
